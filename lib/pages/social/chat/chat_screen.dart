        ...
        children: [
          ...
          GestureDetector(
            onTap: () {
              ...
              MessageController.getController().sendMessage();
            },
            child: Icon(
              Icons.send,
...
class _MessageComponentState extends State<MessageComponent> {
  ...
  @override
  Widget build(BuildContext context) {
    return messageController.amityMessageLength <= 0
        ? Column(
        ...
        : Container(
            ...
            child: ListView.builder(
              ...
              shrinkWrap: true,
              itemCount: messageController.amityMessageList?.length,
              itemBuilder: (context, index) {
                var data = messageController.amityMessageList![index].data;

                bool isSendbyCurrentUser =
                    messageController.amityMessageList?[index].userId !=
                      ...
                      children: [
                        if (!isSendbyCurrentUser)
                          Container(
                            child: Text(
                              getTimeStamp(
                                  messageController.amityMessageList![index]),
                              style:
                                  TextStyle(color: AppColors.grey, fontSize: 8),
                            ),
                          ),
                        messageController.amityMessageList![index].data!.text ==
                                null
                            ? Container(
                          ...
                          Container(
                            child: Text(
                              getTimeStamp(
                                  messageController.amityMessageList![index]),
                              style: TextStyle(
                                  color: AppColors.amity_lightGrey500,
                                  fontSize: 8),
                            ),
                          ),
                      ],
                    ),
                    if (index + 1 == messageController.amityMessageList?.length)
                      const SizedBox(
                        height: 90,
                      )
                  ],
